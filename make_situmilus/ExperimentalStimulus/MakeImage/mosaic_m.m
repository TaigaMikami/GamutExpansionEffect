...
%%%%%%%%%%% 画像の読み込み %%%%%%%%%%%
Ori=imread('./OriginalImage/flower.bmp');    % 原画像
Mk = imread('./OriginalImage/flower.bmp');
...
%%%%%ランダム処理%%%%%%%%%%
sep = 40;  %ランダムパッチの分割数  画素数を割り切れる数にしないとエラーを吐く
...
ave_L = zeros(height/sep,width/sep);
ave_a = zeros(height/sep,width/sep);
ave_b = zeros(height/sep,width/sep);
    ...
    for j1 = 0:sep-1
         ...
         for i = boxst2y:boxen2y
             ...
             for j = boxst2x:boxen2x
                 ...
                 box2a(m,n) = a(i,j);
                 box2b(m,n) = b(i,j);
%                   ave_L = mean2(L(boxst2x:boxen2x,boxst2y:boxen2y));
%                   ave_a = mean2(a(boxst2x:boxen2x,boxst2y:boxen2y));
%                   ave_b = mean2(b(boxst2x:boxen2x,boxst2y:boxen2y));
%                   box2L(m,n) = ave_L;
%                   box2a(m,n) = ave_a;
%                   box2b(m,n) = ave_b;
                 mask_b(m,n) = mask(i,j);
...
%                 j2 = j2 + 1; 
                 ...
                 if (i2 == 280) && (j2 == 280)  %画像が埋まっているかを判断
                     roop = 0;
                 end
             end
...
imwrite(RGB2,'OutputImage/flower_org_moz.bmp','bmp');